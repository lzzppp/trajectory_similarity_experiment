...
class TCAN(nn.Module):
	def __init__(self):
		...
		self.encoder_decoder = S2sTransformer(12345)
	...
	def forward(self, a_gps_data, n_gps_data, f_gps_data,
	            ...
	            anchor_near_len, near_anchor_len, anchor_far_len, far_anchor_len):
		near_anchor_output = self.encoder_decoder(a_gps_data, n_gps_data,
		                                          a_position_data, n_position_data,
		                                          src_mask=a_attention_mask_data, memory_mask=None,
		...
		anchor_near_output = self.encoder_decoder(n_gps_data, a_gps_data,
		                                          ...
		                                          src_mask=n_attention_mask_data, memory_mask=None,
		...
		anchor_far_output = self.encoder_decoder(f_gps_data, a_gps_data,
		                                         ...
		                                         src_mask=f_attention_mask_data, memory_mask=None,
		...
		idx = 0
		for anlen, nalen, aflen, falen in zip(anchor_near_len, near_anchor_len, anchor_far_len, far_anchor_len):
			near_anchor_embedding.append(near_anchor_output[idx//11, nalen, :].unsqueeze(0))
			anchor_near_embedding.append(anchor_near_output[idx//11, anlen, :].unsqueeze(0))
			far_anchor_embedding.append(far_anchor_output[idx//11, falen, :].unsqueeze(0))
			anchor_far_embedding.append(anchor_far_output[idx//11, aflen, :].unsqueeze(0))
			idx += 1
