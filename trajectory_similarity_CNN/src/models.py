...
class OCD(nn.Module):
	...
	def forward(self, x, y):
		x_feature = self.features(x).squeeze()
		y_feature = self.features(y).squeeze()
		
		predict_distance_x_of_y = torch.exp(-F.pairwise_distance(x_feature, y_feature, p=2))
		
		return predict_distance_x_of_y
