...
def get_anchor_batch_pair(trajectory_data, query_index, batch_size):
    print(str(query_index) + " started!")
    index = 0
    ...
    while index < len(trajectory_data):
        ...
        query_input_matrix.append(trajectory_matrixs)
        index += batch_size
    pickle.dump(query_input_matrix, open("query_input_matrix/" + str(query_index) + ".pkl", "wb"))
    print(str(query_index) + " down!")
...
if __name__ == "__main__":
    ...
    test_lat_lon_dataset = pickle.load(open("/home/xiaoziyang/Downloads/trajectory/bert_similarity/porto_test_grid.pkl", "rb"))[:10000]
    ...
    # model.load_state_dict(torch.load("/home/xiaoziyang/Github/trajectory_similarity_matrix_learning/ocd_porto3.pt"))
    
    flag = 0
    pool = multiprocessing.Pool(processes=40)
    
    with torch.no_grad():
        for anchor_index in range(1000):
            pool.apply_async(get_anchor_batch_pair, (test_dataset, anchor_index, 1024))
        pool.close()
        pool.join()