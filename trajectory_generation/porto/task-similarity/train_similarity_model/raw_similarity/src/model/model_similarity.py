...
class TRAJ_ZONE_EMBEDDING(nn.Module):
    ...
    def forward(self, grid, pos, gps, mask):
        ...
        grid_pos_embedding = self.pos_encoder(grid_embedding.transpose(0, 1), pos)
...
class TERT(nn.Module):
    ...
    def forward(self, traj_gps, traj_mask, traj_grid, traj_grid_pos, traj_length, traj_anchor_sub_length, traj_farest_sub_length,
        ...
        traj_tensor, traj_embedding = self.trajectory_encoder(traj_zone_embedding, traj_length)
...
class PAMA():
    def __init__(self):
        self.n_words = 13416
        self.path = "porto_encoder_decoder_27.pth"

para = PAMA()
embed = TRAJ_ZONE_EMBEDDING(para)