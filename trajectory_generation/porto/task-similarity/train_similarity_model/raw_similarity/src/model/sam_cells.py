from src.model.memory import Attention, SpatialExternalMemory
...
class SAM_LSTMCell(RNNCellBase):
    def __init__(self, input_size, hidden_size, grid_size, bias=True, incell = True):
        ...
        if bias:
            ...
            self.register_parameter('bias_ih', None)
            self.register_parameter('bias_hh', None)
        self.spatial_embedding = SpatialExternalMemory(grid_size[0]+3*spatial_width,
                                                       grid_size[1]+3*spatial_width,
                                                       hidden_size).cuda()
    ...
    def spatial_lstm_cell(self, input_a, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
        ...
        input = input_a[:,:-2]
        grid_input = input_a[:,-2:].type(torch.LongTensor).cuda() + spatial_width
    ...
    def batch_update_memory(self, input_a, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
        ...
        # print 'cy size: {}'.format(cy_h.size())
        cy_hh = cy_h.data
        cs = self.spatial_embedding.find_nearby_grids(grid_input, spatial_width)
