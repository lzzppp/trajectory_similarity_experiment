...
class EncoderwithDecoderTrainer(object):
    ...
    def train(self, epoch):
        ...
        for batch in self.get_batch_quick():
            ...
            anch_pos = torch.arange(1, max(batch[9]) + max(batch[10]) + 1, device=torch.device("cuda:0")).unsqueeze(0).expand(self.batch_size, max(batch[9]) + max(batch[10]))
            anch_len = batch[10]
            ...
            fare_pos = torch.arange(1, max(batch[9]) + max(batch[11]) + 1, device=torch.device("cuda:0")).unsqueeze(0).expand(self.batch_size, max(batch[9]) + max(batch[11]))
            fare_len = batch[11]
            ...
            # anchor_dist, farest_dist = torch.FloatTensor(batch[12]), torch.FloatTensor(batch[13])
            # anchor_sub_dist, farest_sub_dist = torch.FloatTensor(batch[18]), torch.FloatTensor(batch[19])
            #############################################
