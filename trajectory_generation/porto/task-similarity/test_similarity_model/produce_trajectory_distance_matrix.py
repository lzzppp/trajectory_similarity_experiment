...
def preprocess_trajectory_dataset(Index_Lat_Lon_Dataset, latitude_unit, longitude_unit, latitude_min, longitude_min):
    ...
    for trajectory_ in tqdm(Index_Lat_Lon_Dataset): # grid lon lat
        # for p in trajectory_[0]:
        gps_trajectory = [[latitude_min + latitude_unit*p[2],
                           longitude_min + longitude_unit*p[1]] for p in trajectory_[0]] # lat lon
...
if __name__ == "__main__":
    ...
    test_dataset = preprocess_trajectory_dataset(test_lat_lon_dataset, lat_unit, lon_unit, porto_range["lat_min"], porto_range["lon_min"])

    np_traj_coord = []
    for trajectory in test_dataset:
        np_traj_coord.append(np.array([point for point in trajectory]))  # lat lon

    np_traj_coord_test = np_traj_coord[:10000]
    ...
    trajecotry_distance_list(np_traj_coord_test, np_traj_coord, batch_size=25, processors=40, distance_type=distance_type,
                             data_name='porto')
