...
def calculate_dtw_dist_path(patha_list, pathb_list):
    dists, paths = [], []
    for i in range(len(patha_list)):
        dist, path = [], []
        for j in range(len(pathb_list)):
            dis, pat = DynamicTimeWarping(patha_list[i], pathb_list[j])
            dist.append(dis)
            path.append(pat)
        dists.append(dist)
        paths.append(path)
