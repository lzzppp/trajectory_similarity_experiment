...
porto_lat_range = [41.003766, 41.47263]
porto_lon_range = [-8.78013, -8.591004]
# porto delta 0.0001
...
class Preprocesser(object):
    ...
    def _traj2grid_preprocess(self, traj_feature_map, isCoordinate = True):
        trajs_hash = []
    ...
    def preprocess(self, traj_feature_map, isCoordinate = True):
        if not isCoordinate:
...
def trajectory_feature_generation(path ='./data/porto/toy_trajs',
                                  lat_range = porto_lat_range,
                                  lon_range = porto_lon_range,
                                  min_length=50):
    fname = path.split('/')[-1].split('_')[0]
    trajs = cPickle.load(open(path, "rb"))
    traj_index = {}
    max_len = 0
    preprocessor = Preprocesser(delta = 0.0001, lat_range = lat_range, lon_range = lon_range)
