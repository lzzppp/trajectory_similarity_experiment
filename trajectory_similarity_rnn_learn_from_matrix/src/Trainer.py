...
class MatrixTrainer(object):
    ...
    def batch_generator(self):
        ...
        while index < self.dataset_length:
            ...
            for i in range(batch_size):
                ...
                anchor_trajectory[i] = [[(p[0] - self.trajectory_feature[0]) / self.trajectory_feature[2] * scale,
                                         (p[1] - self.trajectory_feature[1]) / self.trajectory_feature[3] * scale] for p in anchor_trajectory[i]]

                match_trajectory[i] = [[(p[0] - self.trajectory_feature[0]) / self.trajectory_feature[2] * scale,
                                        (p[1] - self.trajectory_feature[1]) / self.trajectory_feature[3] * scale] for p in match_trajectory[i]]

                unmatch_trajectory[i] = [[(p[0] - self.trajectory_feature[0]) / self.trajectory_feature[2] * scale,
                                          (p[1] - self.trajectory_feature[1]) / self.trajectory_feature[3] * scale] for p in unmatch_trajectory[i]]
    ...
    def train_step(self, epoch):
        ...
        for batch in self.batch_generator():
            # print(batch[0][0])
            # print(batch[1][0])
            # pickle.dump(batch[0][0], open("example_match1.pkl", "wb"))
            # pickle.dump(batch[1][0], open("example_match2.pkl", "wb"))
            # asaddaasda
            ...
            # feature_loss_m = self.loss_fn(feature_predict_data_m, feature_target_data_m)
            # feature_loss_um = self.loss_fn(feature_predict_data_um, feature_target_data_um)

            feature_loss_m = F.kl_div(feature_predict_data_m.softmax(dim=-1).log(), feature_target_data_m.softmax(dim=-1))
            feature_loss_um = F.kl_div(feature_predict_data_um.softmax(dim=-1).log(), feature_target_data_um.softmax(dim=-1))
