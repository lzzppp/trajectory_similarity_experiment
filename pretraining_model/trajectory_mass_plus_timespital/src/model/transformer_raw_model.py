...
from src.model.transformer_compent import transformer
...
class S2sTransformer(nn.Module):
    ...
    def __init__(self,vocab_size,d_model=128,nhead=8,num_encoder_layers=6,
        ...
        # Encoder
        encoder_layer = transformer.TransformerEncoderLayer(d_model, nhead, dim_feedforward, dropout, activation="gelu")
        encoder_norm = nn.LayerNorm(d_model)
        self.encoder = transformer.TransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)

        # Decoder
        decoder_layer = transformer.TransformerDecoderLayer(d_model, nhead, dim_feedforward, dropout, activation="gelu")
        decoder_norm = nn.LayerNorm(d_model)
        self.decoder = transformer.TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm)
