...
class EncoderwithDecoderTrainer(object):
    ...
    def get_batch(self):
        ...
        while self.i < len(self.data):
            ...
            for j in range(self.i, self.i + self.bacth_size):
                # batch_data_Raw = [[1] + self.data[j][0] + [1], [0] + self.data[j][1] + [self.data[j][1][-1] + 15]]
                # batch_data_example = [[1] + self.data[j][0] + [1], [0] + self.data[j][1] + [self.data[j][1][-1] + 15]]
                batch_data_Raw = [1] + self.data[j][0] + [1]
                batch_data_example = [1] + self.data[j][0] + [1]
            ...
            src_mask = [[False]*len1 + [True]*(length1_max - len1) for len1 in length1]
            tgt_mask = [[False]*len2 + [True]*(length2_max - len2) for len2 in length2]
            # pred_mask = [l2*[True] + (length2_max-l2)*[False] for l2 in length2]
            batch_data_token = [line + (length1_max-len(line))*[0] for line in batch_data]
            batch_data2_token = [line + (length2_max-len(line))*[0] for line in batch_data2]
            batch_data_position = [list(range(1, length1_max + 1)) for _ in length1]
            batch_data2_position = [shuffle_position + [0]*(length2_max - len(shuffle_position)) for shuffle_position in position]
            tgt_forward_mask = [[float(0.0)]*i + [float('-inf')]*(length2_max-i) for i in range(1, length2_max + 1)]
            self.i += self.bacth_size
            yield batch_data_token, batch_data2_token, batch_data_position, batch_data2_position, pred_target, src_mask, tgt_mask, tgt_forward_mask, length1
    ...
    def mass_road_step(self, epoch):
        ...
        with torch.no_grad():
            ...
            # dataloader = self.process_dataset()
            # self.data.get_batch()
            for b_d_token, b_d2_token, b_d_position, b_d2_position, p_target, src_mask, tgt_mask, tgt_f_mask, length_list in self.get_batch ():
                ...
                pred_out = self.model (b_d_token, b_d2_token, b_d_position, b_d2_position, tgt_mask=tgt_f_mask,
                                       src_key_padding_mask=src_mask, tgt_key_padding_mask=tgt_mask,
                                       memory_key_padding_mask=src_mask)
                test_list = []
                num_true = 0
                for i, j in zip(list(torch.argmax(pred_out, dim=-1).cpu().numpy()),
                                list(p_target.cpu().numpy())):
                    if i == j:
                        num_true += 1
                ...
                del b_d_token, b_d2_token, b_d_position, b_d2_position, p_target, src_mask, tgt_mask, tgt_f_mask, loss, pred_out
