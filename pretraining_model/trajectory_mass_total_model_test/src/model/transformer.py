...
N_MAX_POSITIONS = 260  # maximum input sequence length
...
class PredLayer(nn.Module):
    ...
    """
    def __init__(self, params, n_words):
        super().__init__()
        self.asm = params.asm
        ...
        if params.asm is False:
            self.proj = Linear(dim, n_words, bias=True)
        else:
    ...
    def forward(self, x, y, y_weight, get_scores=False):
        """
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input, mask, kv=None, cache=None, segment_label=None):
        ...
        q = self.q_lin(input).view(bs, -1, self.n_heads, dim_per_head).transpose(1, 2)
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, dico, is_encoder, with_output):
        ...
        self.n_words = params.n_words
        self.n_words_time = params.n_words
        ...
        # self.embeddings_time = Embedding(self.n_words_time, self.dim, padding_idx=self.pad_index)
        ...
        # output layer
        if self.with_output:
            self.pred_layer = PredLayer(params, self.n_words)
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None, cache=None, enc_mask=None):
        ...
        if self.is_decoder and src_enc is not None:
            ...
            # src_mask = torch.arange(10, dtype=torch.long, device=lengths.device) < src_len[:, None]
        ...
        if positions is None:
            ...
            positions = torch.arange(1, slen+1, out=positions).unsqueeze(0)
        else:
        ...
        # tensor_time = self.embeddings_time(y)
        
        # tensor = tensor + self.position_embeddings(positions).expand_as(tensor) + tensor_time
        ...
        for i in range(self.n_layers):
            ...
            if self.is_decoder and src_enc is not None:
                if self.english_only is True:
                    ...
                    attn = self.encoder_attn[i](tensor, src_mask, kv=src_enc, cache=cache, segment_label=None)
                else:
    ...
    def predict(self, tensor, pred_mask, y, get_scores, target_weight):
        """
        ...
        scores, loss = self.pred_layer(masked_tensor, y, target_weight, get_scores)
        return scores, loss
...
class Trajectory_Mass_Model(nn.Module):
    
    def __init__(self, params, dico):
        super ().__init__()
        self.encoder=TransformerModel(params, dico, is_encoder=True, with_output=True)
        self.decoder=TransformerModel(params, dico, is_encoder=False, with_output=True)
    
    def forward(self, x, y, xp, pt, len1, len2, ptm):
        enc1 = self.encoder('fwd', x=x.transpose(1, 0), lengths=len1,
                            causal=False)
        enc1 = enc1.transpose(0, 1)
    
        #
        enc_mask = x.transpose(0, 1).ne(2)
        enc_mask = enc_mask.transpose(0, 1)
        #
        dec2 = self.decoder('fwd',
                            x=y.transpose(1, 0), lengths=len2, causal=True,
                            src_enc=enc1, src_len=len1, positions=xp.transpose(1, 0), enc_mask=enc_mask)
        #
        _, loss = self.decoder('predict', tensor=dec2, pred_mask=ptm, y=pt, get_scores=False)
