...
class Vector_Embedding(nn.Module):
    def __init__(self, vocab_size):
        ...
        self.gps_linear = nn.Linear(2, 128)
        self.grid_linear = nn.Embedding(vocab_size, 126, padding_idx=0)
    
    def forward(self, gps_grid_data):
        grid_embedding_data = self.grid_linear(gps_grid_data[1])
        embedding_data = torch.cat((gps_grid_data[0], grid_embedding_data), dim=-1)
...
class S2sTransformer(nn.Module):
    ...
    def __init__(self,vocab_size,d_model=128,nhead=8,num_encoder_layers=4,
        ...
        # Preprocess
        self.embedding = Vector_Embedding(vocab_size)
    ...
    def forward(self, src, tgt, src_pos, tgt_pos, src_mask=None, tgt_mask=None,
