...
class RegressSimiTrainer(object):
    ...
    def batch_genarator(self):
        ...
        while self.index < len(self.data):
            ...
            anchor_grid, near_grid, far_grid = [], [], []
            ...
            for i in range(self.index, self.index + batch_size):
                anchor_gps.append(self.data[i][0])
                anchor_grid.append(self.data[i][1])
                anchor_length.append(len(self.data[i][0]))
                near_index = self.get_index(i)
                near_gps.append(self.data[near_index][0])
                near_grid.append(self.data[near_index][1])
                near_length.append(len(self.data[near_index][0]))
                far_index = self.get_index(i, mode='far')
                far_gps.append(self.data[far_index][0])
                far_grid.append(self.data[far_index][1])
                far_length.append(len(self.data[far_index][0]))
                near_distance_matrix = FrechetDistanceLoop.FrechetDistance(np.array(anchor_gps[-1]), np.array(near_gps[-1]))[0] * mail_pre_degree
                far_distance_matrix = FrechetDistanceLoop.FrechetDistance(np.array(anchor_gps[-1]), np.array(far_gps[-1]))[0] * mail_pre_degree

                anchor_near_double = self.get_triple_index(anchor_length[-1], near_length[-1])
                anchor_far_double = self.get_triple_index(anchor_length[-1], far_length[-1])
                # anchor_near_memory_mask.extend (
                #     self.get_memory_mask (anchor_near_double[0], anchor_near_double[1], anchor_length_max,
                #                           near_length_max))
                # anchor_far_memory_mask.extend (
                #     self.get_memory_mask (anchor_far_double[0], anchor_far_double[1], anchor_length_max,
                #                           far_length_max))
                # near_anchor_memory_mask.extend (
                #     self.get_memory_mask (anchor_near_double[1], anchor_near_double[0], near_length_max,
                #                           anchor_length_max))
                # far_anchor_memory_mask.extend (
                #     self.get_memory_mask (anchor_far_double[1], anchor_far_double[0], far_length_max,
                #                           anchor_length_max))
                
                for nei_i in range(11):
                    near_distance.append(math.exp(-near_distance_matrix[anchor_near_double[0][nei_i]][anchor_near_double[1][nei_i]]))
                    far_distance.append(math.exp(-far_distance_matrix[anchor_far_double[0][nei_i]][anchor_far_double[1][nei_i]]))
            ...
            for a_length, n_length, f_length in zip(anchor_length, near_length, far_length):
                ...
                anchor_grid[ind] = anchor_grid[ind] + [0 for aig in range (anchor_length_max - a_length)]
                near_grid[ind] = near_grid[ind] + [0 for nig in range (near_length_max - n_length)]
                far_grid[ind] = far_grid[ind] + [0 for fig in range (far_length_max - f_length)]
            ...
            yield [anchor_gps,anchor_grid], [near_gps,near_grid], [far_gps,far_grid], \
                  anchor_mask, near_mask, far_mask, \
                  anchor_position, near_position, far_position, \
    ...
    def train_step(self, epoch):
        ...
        for batch in self.batch_genarator():
            anchor_gps_data, near_gps_data, far_gps_data = torch.FloatTensor(batch[0][0]).cuda().transpose(0, 1), \
                                                           torch.FloatTensor(batch[1][0]).cuda().transpose(0, 1), \
                                                           torch.FloatTensor(batch[2][0]).cuda().transpose(0, 1)
            anchor_grid_data, near_grid_data, far_grid_data = torch.LongTensor(batch[0][1]).cuda().transpose(0, 1), \
                                                              torch.LongTensor(batch[1][1]).cuda().transpose(0, 1), \
                                                              torch.LongTensor(batch[2][1]).cuda().transpose(0, 1)
            ...
            pred_near_distance, pred_far_distance = self.model([anchor_gps_data,anchor_grid_data], [near_gps_data,near_grid_data], [far_gps_data,far_grid_data],
