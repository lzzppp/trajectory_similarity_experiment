...
class RNNEncoder(nn.Module):
    def __init__(self):
        ...
        self.rnn_encoder = nn.LSTM(input_size=130, hidden_size=128, bidirectional=True, batch_first=True)
        self.memory = nn.Embedding(14000, 128, padding_idx=0)
        self.transform = f_hid_sim(512)
    ...
    def forward(self, gps_data1, grid1, gps_data2, grid2):
        ...
        hidden_cat1 = torch.cat((hidden1[0,:,:], hidden1[1,:,:]), dim=-1)
        hidden_cat2 = torch.cat((hidden2[0,:,:], hidden2[1,:,:]), dim=-1)
        
        hidden = torch.cat((hidden_cat1, hidden_cat2), dim=-1)
...
class OCD(nn.Module):
    ...
    def forward(self, x, gps1, grid1, gps2, grid2):
        ...
        # x = torch.exp(-x)
        return x, feature_target, feature_predict
